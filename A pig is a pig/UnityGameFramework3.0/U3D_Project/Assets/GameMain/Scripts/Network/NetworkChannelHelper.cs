...
namespace StarForce
{
    public class NetworkChannelHelper : INetworkChannelHelper
    {
        ...
        public bool Serialize<T>(T packet, Stream destination) where T : Packet
        {
            ...
            CSPacketBase csPacketBase = packet as CSPacketBase;
            csPacketBase.ToStream(destination);
            ReferencePool.Release((IReference)csPacketBase);
            return true;
        }
        ...
        public IPacketHeader DeserializePacketHeader(Stream source, out object customErrorData)
        {
            ...
            source.Read(headLen, 0, headLen.Length);
            sCPacketHeader.PacketLength = BitConverter.ToInt32(headLen,0);
            return (IPacketHeader)sCPacketHeader;
        }
        ...
        public Packet DeserializePacket(IPacketHeader packetHeader, Stream source, out object customErrorData)
        {
            ...
            if (scPacketHeader.IsValid)
            {
                ...
                if (packetType != null)
                {
                    ...
                    (packet as SCPacketBase).Decode();
                }
                else
                {
                    Log.Warning("Can not deserialize packet for packet id '{0}'.", id.ToString());
                }
            }
            else
            {
        ...
        SocketEnum socketEnum;
        public void SetServerType(SocketEnum socketEnum)
        {
        ...
        {
            GameEntry.Event.Fire(SocketConnectEventArgs.EventId,SocketConnectEventArgs.Create(socketEnum));
