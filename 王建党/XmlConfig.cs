...
public class XmlConfig : Singleton<XmlConfig>
{
    ...
    {
        string path = Application.streamingAssetsPath + "/Server.xml";
        //Debug.Log(path);
        if (!File.Exists(path))
        {
            //创建
    ...
    {
        string path = Application.dataPath + "/OutPath/SceneLayer1.xml";
        //Debug.Log(path);
        if (!File.Exists(path))
        {
            //创建
    ...
    public void LoadXml()
    {
        //创建xml文档
        XmlDocument xml = new XmlDocument();
        xml.Load(Application.streamingAssetsPath + "/Server.xml");
        //得到objects节点下的所有子节点
        XmlNodeList xmlNodeList = xml.SelectSingleNode("Server").ChildNodes;
        Debug.Log("Server数量：" + xmlNodeList.Count);
        //遍历所有子节点
        foreach (XmlElement xl1 in xmlNodeList)
        {
            Debug.Log("服务器id：" + xl1.GetAttribute("id"));
            Debug.Log("服务器ip：" + xl1.GetAttribute("serverIP"));
            Debug.Log("服务器端口号：" + xl1.GetAttribute("serverPort"));
            Debug.Log("服务器名称：" + xl1.GetAttribute("serverName"));
    ...
    public void LoadSceneXml()
    {
        //创建xml文档
        XmlDocument xml = new XmlDocument();
        xml.Load(Application.dataPath + "/OutPath/SceneLayer1.xml");
        //得到objects节点下的所有子节点
        XmlNodeList xmlNodeList = xml.SelectSingleNode("Scene").ChildNodes;
        Debug.Log("Layer数量：" + xmlNodeList.Count);
        //遍历所有子节点
        foreach (XmlElement xl1 in xmlNodeList)
        {
            Debug.Log("场景物体id：" + xl1.GetAttribute("id"));
            Debug.Log("场景物体loadPath：" + xl1.GetAttribute("loadPath"));
            Debug.Log("场景物体position：" + xl1.GetAttribute("position"));
            Debug.Log("场景物体type：" + xl1.GetAttribute("type"));
            Debug.Log("场景物体width：" + xl1.GetAttribute("width"));
